...
document.getElementById('signature-save').onclick = () => {
    ...
    // Initialize arrays
    if (!allPageEdits[pageNum].signatures) {
        allPageEdits[pageNum].signatures = [];
    }
    if (!allPageEdits[pageNum].overlays) {
        allPageEdits[pageNum].overlays = [];
    }
    ...
    allPageEdits[pageNum].signatures.push(signatureData);
    
    // Also add to overlays for display
...
window.checkSavedData = function() {
    ...
    console.log("allPageEdits:", allPageEdits);
    
    if (allPageEdits && allPageEdits[pageNum]) {
        console.log(`Page ${pageNum} data:`, allPageEdits[pageNum]);
        
        if (allPageEdits[pageNum].textHighlights) {
            console.log(`Text Highlights: ${allPageEdits[pageNum].textHighlights.length}`);
        }
        
        if (allPageEdits[pageNum].shapes) {
            console.log(`Shapes: ${allPageEdits[pageNum].shapes.length}`);
        }
        
        if (allPageEdits[pageNum].textEdits) {
            console.log(`Text Edits: ${allPageEdits[pageNum].textEdits.length}`);
        }
        
        if (allPageEdits[pageNum].images) {
            console.log(`Images: ${allPageEdits[pageNum].images.length}`);
        }
    }
